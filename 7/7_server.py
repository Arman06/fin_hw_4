...
from super_file_manager import FileManager
import os
...
class MySuperFTPServer:
    def __init__(self, host, port, logger_filename='log.txt'):
        self.host = host
        self.port = port
        self.server_socket = None
        self.client_connections = []
        self.logger = MyLogger(os.path.abspath(logger_filename))
        self.users_path = os.path.abspath('./users.json')
    ...
    def handle_accepting(self):
        try:
            while True:
                ...
                user = self.check_user(my_client_socket.receive_message_with_header())
    ...
    def start(self):
        ...
        accepting_thread = threading.Thread(target=self.handle_accepting)
    ...
    def check_user(self, name):
        try:
            with open(self.users_path, "r") as file:
                ...
                for user in users:
                    if str(user['name']) == str(name.strip()):
                        return user
    ...
    def auth_user(self, user, password):
        ...
        salted_password = password + salt
        hashed_password = hashlib.sha256(salted_password.encode()).hexdigest()
        return str(hashed_password) == str(user['password'])
    ...
    def handle_client(self, client_socket, user, address):
        ...
        #     path = f.read().strip()
        if not os.path.exists(f"./usrs/{user['name']}"):
            os.makedirs(f"./usrs/{user['name']}")
        fm = FileManager(f"./usrs/{user['name']}")
        client_socket.send_message_with_header(fm.list_commands())
        while True:
            data = client_socket.receive_message_with_header()
            if not data:
                break
            client_socket.send_message_with_header(fm.execute_commands(data))

            msg = f'{user["name"]} использует ---> {data}'
            self.logger.make_an_entry(msg)
            # for client in self.client_connections:
            #     client.send_message_with_header(f'{user["name"]}> {data}')
            #     msg = f"Отправка данных клиенту: {user['name']}> {data}"
            #     self.logger.make_an_entry(msg)
...
if __name__ == "__main__":
    server = MySuperFTPServer("localhost", 8000)
    server.start()
